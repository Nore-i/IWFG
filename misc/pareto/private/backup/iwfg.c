...
int iwfg_bottom_k(const FRONT *in_front, int k, int *idx_out)
{
    ...
    for (int i = 0; i < m; ++i)
    {
        ...
        printf("Expoint %d: ", expoints[i].idx);
        printf("nslices = %d, nslices_alloc = %d, done_slices = %d\n",
            expoints[i].nslices, expoints[i].nslices_alloc, expoints[i].done_slices);
...
{    
    ...
    printf("Expoint %d has done how many slices: %d/%d\n", ep->idx, ep->done_slices, ep->nslices);
    printf("Buffer allocated #points: %d, allocated #objectives: %d\n", buffer->nPoints_alloc, buffer->n_alloc);
    printf("Slice #points: %d, #objectives: %d\n", np_slice, dim_slice);
